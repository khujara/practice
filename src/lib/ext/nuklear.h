...
*/
struct nk_user_FontGlyph;
typedef float(*nk_text_width_f)(nk_handle, float h, const char*, int len);
typedef void(*nk_query_FontGlyph_f)(nk_handle handle, float font_height,
                                    struct nk_user_FontGlyph *glyph,
...
struct nk_font {
    ...
    float scale;
    struct nk_FontGlyph *glyphs;
    const struct nk_FontGlyph *fallback;
...
NK_API const struct nk_FontGlyph* nk_font_find_glyph(struct nk_font*, nk_rune unicode);
...
nk_font_bake(struct nk_font_baker *baker, void *image_memory, int width, int height,
    struct nk_FontGlyph *glyphs, int glyphs_count,
...
#ifdef NK_INCLUDE_VERTEX_BUFFER_OUTPUT
NK_INTERN void
nk_font_query_FontGlyph(nk_handle handle, float height,
    struct nk_user_FontGlyph *glyph, nk_rune codepoint, nk_rune next_codepoint)
{
    float scale;
    const struct nk_FontGlyph *g;
    struct nk_font *font;
...
NK_API const struct nk_FontGlyph*
nk_font_find_glyph(struct nk_font *font, nk_rune unicode)
{
    int i = 0;
    int count;
    int total_glyphs = 0;
    const struct nk_FontGlyph *glyph = 0;
    NK_ASSERT(font);
    NK_ASSERT(font->glyphs);
...
nk_font_init(struct nk_font *font, float pixel_height,
    nk_rune fallback_codepoint, struct nk_FontGlyph *glyphs,
...
#ifdef NK_INCLUDE_VERTEX_BUFFER_OUTPUT
    font->handle.query = nk_font_query_FontGlyph;
    ...
    enum nk_font_atlas_format fmt)
{
    ...
    atlas->glyphs = (struct nk_FontGlyph*)
        atlas->permanent.alloc(atlas->permanent.userdata,0,
                sizeof(struct nk_FontGlyph) * (nk_size)atlas->glyph_count);
